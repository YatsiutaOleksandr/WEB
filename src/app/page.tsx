...
export default async function HomePage() {
  ...
  return (
    <div>
      ...
      <p className={styles["stations-desc"]}>Here you can see all monitoring stations and their current air quality.</p>
      <div className={styles["stations-list"]}>
        {data.map((s: any) => (
          <div key={s.id} className={styles["station-card"]}>
            ...
            <div className={styles["station-value"]}>{s.value ?? 45}</div>
            <Link href={`/station/${s.id}`} className={styles["station-link"]}>View Details →</Link>
          </div>
        ))}
      </div>
    </div>
